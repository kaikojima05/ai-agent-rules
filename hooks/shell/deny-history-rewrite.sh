...
# 契約: コミット履歴の書き換えは禁止。唯一の例外は rebase スキルの決定的スクリプト
...
echo "$MASKED" | grep -qE '(^|[;&|[:space:]])git[[:space:]]+rebase([[:space:]]|$)' && \
  hook_deny "git rebase は履歴書き換えのため禁止です。コミットの整理は rebase スキル（決定的スクリプト）経由でのみ行ってください。"
...
# rebase 成功後の安全網は reflog だけになるので、これらを消す操作は例外なく止める
