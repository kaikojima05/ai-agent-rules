...
while IFS= read -r LINE; do
  ...
  BIN="${1:-}"; SUB="${2:-}"
  case "$BIN" in
    npx|pnpx|bunx) hook_deny "$FETCH" ;;
    yarn)
      case "$SUB" in
        ""|add|install|up|upgrade|upgrade-interactive|global|import) hook_deny "$INST" ;;
        dlx|create) hook_deny "$FETCH" ;;
      esac ;;
    npm|pnpm|bun)
      case "$SUB" in
        i|install|ci|add|update|up) hook_deny "$INST" ;;
        x|exec|dlx|create) hook_deny "$FETCH" ;;
      esac ;;
  esac
done <<EOF
$(echo "$MASKED" | sed -E 's/&&/\n/g; s/\|\|/\n/g' | tr '|;' '\n')
