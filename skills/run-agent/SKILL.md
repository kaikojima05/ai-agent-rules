...
この確認の直後、index更新と最終報告の前に `clean-code` を自動で実行する。機能名と、この設計書で変更してコミットした追跡済み本体コードの相対パス一覧を必ず渡す。`clean-code` はその一覧を DeepSeek のネスト検出へ渡すため、上位モデルが改めて差分探索して対象を補完してはならない。`clean-code` の品質ゲートが完了しなければ、index更新と最終報告へ進まない。
