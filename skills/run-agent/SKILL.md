---
name: run-agent
description: "@.[agent_name]/prompt/ の承認済み設計書を1枚選び、上位モデル主導・DeepSeek実装担当のTDDフローで完了させる"
...
CodexまたはClaude Codeを唯一のオーケストレーターとして、未実装の設計書を1枚だけ完了する。DeepSeekには調査または許可済み本体コードの候補パッチ作成だけを委任する。以降、CodexまたはClaude Codeを「上位モデル」と呼ぶ。
...
設計書の対象ファイルを、テスト資産と本体コードへ分類する。DeepSeekへ渡せるのは本体コードだけである。次は上位モデル管理のため委任しない。
...
1. 上位モデルがテストシナリオを提案し、ユーザーの一括承認を得る
2. 上位モデルがテストを書き、ファイル単位で即コミットする
3. 上位モデルがRedを確認する
4. 固定実行器でDeepSeekへ本体コードを委任する
5. 上位モデルが候補パッチを検証し、1ファイルずつ反映・即コミットする
6. 上位モデルがGreen、レビュー、必要な本体コード修正を完了する

調査・ドキュメントだけの設計書は上位モデルが通常どおり実行し、DeepSeek実装を呼ばない。

### 4. DeepSeekの相談を処理する

DeepSeekはテストの穴を上位モデルへ相談できるが、テスト・設計を変更できない。上位モデルが検証し、既存の承認内容で一意に解決できなければ、次へ進まずユーザーへ新しいシナリオまたは設計変更を提示する。
